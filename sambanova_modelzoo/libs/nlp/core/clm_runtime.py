...
from sambanova_modelzoo.libs.nlp.core.attention_mask_utils import get_collapsed_article_attention_mask
...
class PytorchGenerationInputNames(Enum):
    ...
    model.generate(), the input names defined here also need to redirected to model() function as the keyword arguments.
    For example, last_token_index is a ModelZoo specific arguments that also exists in the SNLlamaForCausalLM class's
    forward() function call signature.

    All the string names here need to match the common SN model's keyword arguments for forward() function.
    """
    input_ids = 'input_ids'
...
class SambaGenerationInputNames:
    ...
    Input names have to be unique during tracing between cache gen and token gen call schedule inside the same PEF. 
    ...
    input_ids: Different region names between cache gen and token gen graph because they have different shapes
    attention_mask: Different region names between cache gen and token gen graph because they have different shapes
    last_token_index: Same region name shared between cache gen and token gen graph since they have the same shape
    """
    def deduplicate(self, name: str, **kwargs) -> str:
        """
        Args:
            name: Original name.
            kwargs: Keyword argments that's used to deduplicate the name.
        Returns:
    ...
    @property
    def logits(self) -> str:
        """ Retrurns the logits input name for postprocess graph """
        return PytorchGenerationInputNames.logits.value

    @property
    def last_hidden_states(self) -> str:
        """ Retrurns the last_hidden_states input name for fast postprocess graph """
        return PytorchGenerationInputNames.last_hidden_states.value

    @property
    def generated_tokens(self) -> str:
        """
        Returns the generated_tokens input and output name for postprocess graph. It will be updated in-place to hold
...
class SambaGraphNames:
    ...
    def cache_gen(static_sequence_length: int) -> str:
        """
        Args:
            static_sequence_length: The sequence for the input ids to pad to. This can be shorter than max_seq_length.
        Returns:
    ...
    def token_gen(max_seq_length: int) -> str:
        ...
        Returns:
            Name of graph generated by the continuous token generation pass. This name is compiled into the PEF and then 
...
def is_left_padded(input_ids: torch.Tensor, pad_token_id: int) -> bool:
    ...
    Returns:
        True if any row in `input_ids` is left padded. A row of full padding token does not count as left padded since it
        can be used to pad batch dim for static batch size pef
    """
...
class CachedInferenceRuntime:
    ...
    def __init__(self,
                 ...
                 runner: str = 'pytorch'):
        """
        Base class for preprocessing inputs to run cached inference. The class is stateful in the sense that the first
    ...
    def apply_sliding_window(self):
        """
        Apply sliding window attention mask used in Mistral for example. Sliding window attention only attends the
        latest N tokens.
        """
    ...
    def preprocess_inputs_for_cache_gen(self,
        ...
        """
        Preprocess inputs for cache generation phase, e.g. the first token generation. Becuase we assume right padding,
        ...
        Args:
            input_ids: Left aligned (right-padded) inputs ids of size (batch_size, input_seq_length).
                       input_seq_length should be smaller than compiled static sequence length.
        ...
        expensive_error_if(ValueError,
                           f"attention_mask: {attention_mask} cannot be left padded or contains sliding_window mask",
        ...
        # Use the last 1 in attention mask to indicate the last token. Some tokenizer may use padding token for EOS
        # which make it impossible to find the last token from right-padedd input_ids
    ...
    def preprocess_inputs_for_token_gen(self, input_ids: torch.Tensor) -> Dict[str, torch.Tensor]:
        """
        Preprocess inputs for token generation phase. Token generation phase is the following calls after cache generation.

        For samba frontend, the returned input names must be consistent with the inputs names during tracing (clm_tracer).

        Preprocessings on the inputs include:
        input_ids: Slicing out the last non-pad token according to last_token_index so token generation model pass always
                   takes input_ids of sequence length of 1.
...
class CausalLMGenerationMixin:
    ...
    def init_inputs_processor(self, runner: Optional[str] = 'pytorch', static_seq_lengths: Optional[Set[int]] = None):
        ...
        Args:
            ...
            runner: One of ['pytorch', 'samba']. Pytorch runner returns the inputs with names matching the model forward
                    call's function signature (keyword argument). Samba runner return the inputs with names matching
    ...
    def runner(self, val: str) -> str:
        """
        Setting the runner of the runtime inputs processor
        """
    ...
    def sn_prepare_inputs_for_generation(self,
                                         ...
                                         **kwargs) -> Dict[str, Any]:
        """
        The function replaces Huggingfacce's prepare_inputs_for_generation() function. It is called for
        ...
        This ovewrite ONLY works with right padded tokenization. The outputs are fed directly into ModelZoo model's 
        forward pass. An exception will be raised if the input_ids is left padded. Because Hugging Face always append
        the genrated token to the input token ids, we have to return last_token_index to feed into the model.  
        ...
        if self.inputs_processor.preprocess_inputs_for_cache_gen.calls == 0:
        ...
        # as it's already in RDU memory sharing the same address as the output of cache generation graph. pas_key_values
        # are modified in place in the token generation passes.
        return model_inputs
...
class PretrainRuntime:
    ...
    def shift_and_pad_tensor(self, input_tensor: torch.Tensor) -> torch.Tensor:
        ...
        Args:
            input_tensor: A tensor that is [bs, dynamic_input_length]
        Returns:
            The input_tensor has the first value of the the second dimension removed, the resulting tensor is [bs, dynamic_input_length-1], this is right padded to [bs, max_seq_length]
        """
        assert input_tensor.dim() == 2
    ...
    def prepare_inputs_to_train(self, input_ids: torch.Tensor, token_type_ids: torch.Tensor) -> Dict[str, torch.Tensor]:
        ...
        Returns:
            ...
            labels: The input_tensor has the first value of the the second dimension removed, the resulting tensor is [bs, dynamic_input_length-1], this is right padded to [bs, max_seq_length] 
        """
        return {
