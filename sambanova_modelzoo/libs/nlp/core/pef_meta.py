...
def to_pef_meta_dict(cfg: SambaConfig, recompile_args_only: bool = True) -> Dict[str, Any]:
    ...
    TODO: RK>>We need more info on when recompile is needed
    ...
    Returns:
        A nested dict. Top-level keys are Samba compile arguments (all Samba args are included)
        and dict[APP_ARGS_KEY] are the app-level Pydantic model args
    """
...
def update_common_samba_args_from_pef(run_model: RunArgs) -> RunArgs:
    ...
    Only update fields found in Samba's CommonArgs model, because they are common to CompileArgs & RunArgs.
