...
class BaseCompiler:
    ...
    def __init__(self,
        ...
        if self.model.config.max_seq_length is None:
            raise RuntimeError(f"max_seq_length is None, it is a SNPretrainedConfig and needs to be set to define a static graph on RDU")
...
class CachedInferenceCompiler(BaseCompiler):
    def __init__(self, model: torch.nn.Module, batch_size: int, static_seq_lengths: Optional[Set[int]] = None):
        ...
        self.model_with_postprocess_init = ModelWithPostprocessInit(self.model, batch_size, model.config.vocab_size)
    ...
    def trace(self):
        ...
        for seq_length in self.static_seq_lengths:
            ...
            self.cache_gen_outputs[seq_length] = trace_multigraph(
                self.model_with_postprocess_init, {'cache_gen_inputs': self.cache_gen_inputs[seq_length]},
                trace_prefix=self.graph_names.cache_gen(seq_length))
    ...
    def _connect_postprocess_inplace_updates(self, postprocess_outputs: Dict[SamplingMethod, Tuple[SambaTensor, ...]]):
        ...
        """
        for tokens, last_token_index, attention_mask, generated_tokens, generated_index, token_count in postprocess_outputs.values(
        ):
