...
def sn_llama_decoder_layer_forward(
        self,
        hidden_states: torch.Tensor,
...
def sn_patch_llama_model_add_hyperfunction_annotation(self, config):
    # [SambaNova] For O1 heuristic annotation
    self.hyperfunction = LlamaHyperfunction(config)

def sn_patch_llama_for_causal_lm_add_hyperfunction_annotation(self: 'SNLlamaForCausalLM', config):
    # [SambaNova] For O1 heuristic annotation
    self.hyperfunction = LlamaHyperfunction(config)
...
OVERWRITE_METHODS = {
    'LlamaForCausalLM': [('forward', sn_llama_for_causal_lm_forward)],
    'LlamaRMSNorm': [('forward', sn_llama_rms_norm_forward)],
    'LlamaRotaryEmbedding': [('forward', sn_llama_rotary_embedding_forward)],
    'LlamaAttention': [('forward', sn_llama_attention_forward)],
    'LlamaDecoderLayer': [('forward', sn_llama_decoder_layer_forward)],
    'LlamaModel': [('forward', sn_llama_model_forward)],
